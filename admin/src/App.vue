...
const menuGroupExpanded = ref()

const currentUser = ref(null)

onMounted(async () => {
  const headers = { Authorization: `Bearer ${idToken}` }
  currentUser.value = await fetch('http://localhost:3000/me', { headers }).then((data) => data.json())
})
</script>
            ...
            <ul class="mb-6 flex flex-col gap-1.5">
              ...
              <li>
                <a class="group relative flex items-center gap-2.5 rounded-sm px-4 py-2 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4"
                  href="#" @click.prevent="menuGroupExpanded = (menuGroupExpanded === 'Users' ? '' : 'Users')"
                  :class="{ 'bg-graydark dark:bg-meta-4': (menuGroupExpanded === 'Users') || (page === 'users' || page === 'users-new' || page === 'users-edit' || page === 'user') }">
                  ...
                  <svg class="absolute right-4 top-1/2 -translate-y-1/2 fill-current"
                    :class="{ 'rotate-180': (menuGroupExpanded === 'Users') }" width="20" height="20"
                ...
                <div class="translate transform overflow-hidden"
                  :class="(menuGroupExpanded === 'Users') ? 'block' : 'hidden'">
            ...
            <div class="relative" x-data="{ dropdownOpen: false }" @click.outside="dropdownOpen = false">
              <a class="flex items-center gap-4" href="#" @click.prevent="dropdownOpen = !dropdownOpen">
                <span class="hidden text-right lg:block">
                  <span class="block text-sm font-medium text-black dark:text-white">{{ currentUser?.name }}</span>
                  <span class="block text-xs font-medium">{{ currentUser?.username }}</span>
                </span>
              ...
              <div x-show="dropdownOpen"
                class="absolute right-0 mt-4 flex w-62.5 flex-col rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark hidden">
